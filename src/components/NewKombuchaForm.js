...
function NewKombuchaForm(props) {
  function handleNewKombuchaFormSubmission(event) {
    ...
    props.onNewKombuchaCreation({name: event.target.name.value, brand: event.target.brand.value, price: event.target.price.value, flavor: event.target.flavor.value, quantity: parseInt(event.target.quantity.value), id: v4()});
  }
